...
REST_ROUTER.prototype.handleRoutes= function(router,connection,md5) {
            	...
            	connection.query("SELECT LAST_INSERT_ID()", function(err,rows){
            		...
            		else
            		{
            			...
            			switch(req.body.type){
	            			case "picture_post":
	            			{	
	            				var publicImagePath = "46.101.242.198:3000/" + imageName;
    ...
    router.post("/comment_sent",function(req,res){
        var query = "INSERT INTO ??(??,??,??,??,??) VALUES (?,?,?,?,?)";
        //console.log(req.body.name,req.body.email,req.body.password);
        var table = ["comments", "body", "submitter", "post_id", "anonymous", "parent_id", req.body.body, req.body.submitter, req.body.post_id, req.body.anonymous, req.body.parent_id];
        query = mysql.format(query,table);
        connection.query(query,function(err,rows){
            if(err) {
                res.json({"Error" : true, "Message" : err});
